...
std::vector<uint8_t> BaseStationController::ReadCharacteristicValue(const std::string& charPath)
{
    ...
    // ReadValue takes an (empty) options dictionary.
    ...
    DBusError err;
    dbus_error_init(&err);
    DBusMessage* reply = dbus_connection_send_with_reply_and_block(client->Raw(), msg, 5000, &err);
    dbus_message_unref(msg);

    if (dbus_error_is_set(&err))
    {
        std::cerr << "D-Bus error reading characteristic: " << err.name << " - " << err.message
                  << std::endl;
        dbus_error_free(&err);
        return value;
    }
    ...
    if (dbus_message_iter_init(reply, &replyIter) &&
        dbus_message_iter_get_arg_type(&replyIter) == DBUS_TYPE_ARRAY)
    {
...
std::string BaseStationController::FindChannelCharacteristic()
{
    ...
    std::string charPath = FindCharacteristicPath(servicePath, V2_CHANNEL_CHAR_UUID);
    if (!charPath.empty())
    {
        return charPath;
    }

    // Unknown firmware layout: list what the station does expose, so the
    // characteristic can be identified from a bug report instead of guessed.
    std::cerr << "Channel characteristic " << V2_CHANNEL_CHAR_UUID << " not found on "
              << stationInfo.name << ". Characteristics on this station:\n";
    for (const auto& [path, interfaces] : client->GetManagedObjects())
    {
        if (path.rfind(servicePath + "/", 0) != 0)
        {
        ...
        if (const std::string* uuid = it->second.GetString("UUID"))
        {
            std::cerr << "  " << *uuid << "\n";
        }
    }
...
bool BaseStationController::SetChannel(int channel)
{
    ...
    {
        std::cerr << "Channel " << channel << " out of range ("
